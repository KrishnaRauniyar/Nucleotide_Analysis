...
def protein_freq_table(directory, header):
    protein_data = {}
    all_keys = set()
    ...
    if header == 'yes':
        ...
        with open('localFeatureVect_theta29_dist18_NoFeatureSelection_keyCombine0_header.csv', 'w', newline='') as csvfile:
        ...
        with open('localFeatureVect_theta29_dist18_NoFeatureSelection_keyCombine0.csv', 'w', newline='') as csvfile:
            for protein_name, data in protein_data.items():
            # Without header code
...
def main(opt_dict):
    protein_freq_table(directory= opt_dict['proteins_path'], header= opt_dict['header_bool'])
